    ...
    try {
      ...
      const bookTimes = db.prepare(`
        SELECT d.book_id AS id, COALESCE(MAX(b.title), MAX(d.book_title), '已删除书籍') AS title,
               SUM(d.seconds) AS total_seconds, MAX(d.updated_at) AS last_read_at
      ...
      json(res, 200, { today, today_seconds: todaySeconds, total_seconds: totalSeconds, ...streaks, daily, books, notes });
